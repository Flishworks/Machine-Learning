...
class SquaredLoss(Loss):
    ...
    def forward(self, X, w, y):
        ...
        L_s = []
        for i in range(N):
            L_s.append(.5*(y[i] - np.sum(w.T*X[i]))**2)
        loss = np.mean(np.array(L_s))
    ...
    def backward(self, X, w, y):
        ...
        N = len(X)
        for i in range(N):
            grad += (y[i] - np.sum(w.T*X[i]))*X[i]*-1
        if self.regularization:
            grad += self.regularization.backward(w)
        return grad/N
...
class HingeLoss(Loss):
    ...
    def forward(self, X, w, y):
        ...
        L_h = 0
        for i in range(N):
            if (1-y[i]*np.sum(w.T*X[i])) > 0:
                L_h += 1-y[i]*np.sum(w.T*X[i])
        loss = L_h/N
        return loss
